...
description: "DeepAgents SDK의 핵심 함수와 백엔드를 분석하고, DeepCoWork가 이를 어떻게 래핑하는지 실제 코드로 해부합니다."
tags:
  - deep-agents
  - langchain
  - langgraph
...
> **TL;DR**: DeepCoWork의 에이전트 로직은 `agent_core.py` 단일 파일에 집중되어 있다. `create_deep_agent`가 LangGraph ReAct 그래프를 생성하고, `LocalShellBackend`가 파일/셸 도구를 자동 제공하며, `interrupt_on`이 위험한 도구 호출 전에 실행을 멈춘다. 이 파일이 유일한 SDK 결합 지점이다.
