...
1. 실험 B: gemma-embed-300m 고정 × 약 30 LLM 비교
2. 리랭커 도입 실험 (Qwen3-Reranker, BCE, BGE reranker)
3. Hard 50개 질문 대상: 비전 임베딩 + OCR 전처리로 구제 가능성 평가
4. RAGAS 기반 LLM-as-judge로 답변 품질 측정

모든 원본 JSON은 [GitHub RAG-Evaluation 저장소](https://github.com/baem1n/RAG-Evaluation)의 `results/phase4_embedding/`에 공개한다.
