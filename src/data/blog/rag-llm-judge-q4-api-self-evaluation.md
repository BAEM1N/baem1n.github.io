...
- **실험**: 34개 API LLM 답변을 동일 API judge 8종으로 cross-judge.
...
**retrieval 파이프라인은 모든 quadrant 에서 동일**. 변수는 **생성모델(cand) 과 판단모델(judge) 만**.
...
| VectorStore   | FAISS                       |
| **Embedding** | **gemma-embed-300m (768d)** |
| Retrieval     | top-5 cosine                |

→ 같은 query 가 모든 34 cand LLM 에 **동일한 retrieved chunks** 로 들어가므로 답변 차이 = **순수 LLM 답변 능력 차이**.

Q4 의 변수: **cand = 34 API LLM**, **judge = 8 API LLM** (Sonnet 가족 자기 채점 bias 검증 의도 포함).
