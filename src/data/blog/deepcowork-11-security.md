...
description: "AI 에이전트 데스크톱 앱에서 구현한 보안 조치들을 체크리스트 형태로 정리한 실전 가이드."
tags:
  - security
  - cors
  - path-traversal
...
> **TL;DR**: 경로 탈출 방지, CORS 화이트리스트, 파일 크기 제한, HITL 승인 등 9개 레이어의 다층 방어로 AI 에이전트의 파일 시스템 접근을 제어한다.

## Table of contents

## 위협 모델

[OWASP Top 10](https://owasp.org/www-project-top-ten/)과 AI 에이전트 특화 위협을 조합하여 위협 모델을 정리했다. AI 에이전트 앱의 주요 위협:
...
로컬호스트와 Tauri 전용 오리진만 허용한다. [Tauri 보안 문서](https://v2.tauri.app/security/)의 권고에 따라 외부 웹사이트에서 API를 호출할 수 없도록 제한한다.
...
| 항목 | 수치 |
|------|------|
| is_safe_path() 검증 오버헤드 | ~0.1ms/호출 |
| CORS 허용 오리진 수 | 4개 (localhost 변형 + Tauri) |
| 파일 크기 제한 (일반) | 10MB |
| 메모리 파일 크기 제한 | 50KB |
| 셸 명령 타임아웃 | 60초 |
| 스킬 이름 정규식 검증 | `^[a-z0-9][a-z0-9-]{0,62}[a-z0-9]?$` |
