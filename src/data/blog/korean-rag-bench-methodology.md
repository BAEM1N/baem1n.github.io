...
judge 한 종만 믿으면 위험하다. 46개 generator 리더보드는 **18 judge(오픈 9 + 클로즈 9)** 다수결로 채점했고, 대시보드·384 cartesian에선 judge를 **20종(오픈 11 + API 9)**으로 확장해 교차 검증했다. (절대 점수는 judge에 따라 출렁이지만 조합 간 상대 순위는 대체로 보존된다. 이 견고성 얘기는 생성·judge 편에서 따로 다룬다.)
...
A. 46-generator 리더보드는 18 judge(오픈 9 + 클로즈 9) 다수결로, 대시보드·cartesian은 20종(오픈 11 + API 9)으로 확장해 교차 채점했다. 절대 점수는 judge마다 다르지만 조합 간 상대 순위는 대체로 보존된다.
